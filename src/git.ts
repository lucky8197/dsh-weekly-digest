...
function isSinceArg(value: string): boolean {
...
export function validateGitArgs(args: readonly string[]): boolean {
  if (args.length === 5
    && args[0] === 'log' && args[1] === '--since' && isSinceArg(args[2] ?? '')
    && args[3] === '--date=short' && args[4] === LOG_FORMAT) {
    return true
  }
  if (args.length === 2 && args[0] === 'rev-parse' && args[1] === '--is-inside-work-tree') return true
  if (args.length === 2 && args[0] === 'rev-parse' && args[1] === '--show-toplevel') return true
  if (args.length === 3 && args[0] === 'log' && args[1] === '-1' && args[2] === '--format=%H') return true
  return false
}
...
/** 执行只读 git 命令。 */
export function runGit(cwd: string, args: readonly string[], signal?: AbortSignal): Promise<GitResult> {
