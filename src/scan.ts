...
/** 按日期聚合提交数。 */
export function perDay(commits: CommitInfo[]): { date: string; count: number }[] {
...
/** 按作者聚合提交数（降序）。 */
export function byAuthor(commits: CommitInfo[]): { name: string; count: number }[] {
  const map = new Map<string, number>()
...
export async function scanSessions(sessionsRoot: string, days: number, signal?: AbortSignal): Promise<SessionStats> {
  const cutoff = Date.now() - days * 86_400_000
  const projects: SessionStats['projects'] = []
  let totalSessions = 0
  let totalBytes = 0
  let entries
  try {
...
export async function scanMemories(memoriesRoot: string, days: number, maxEntries: number, maxReadBytes: number, signal?: AbortSignal): Promise<MemoryStats> {
  const cutoff = Date.now() - days * 86_400_000
  const out: MemoryDay[] = []
  let totalEntries = 0
  let files: string[]
  ...
  for (const file of files) {
    ...
    try {
      ...
      const st = await stat(abs)
      if (st.size > maxReadBytes) continue
...
/** 生成 Markdown 周报。 */
export function buildDigest(report: WeeklyDigestReport, maxEntries: number): string {
  const lines: string[] = []
